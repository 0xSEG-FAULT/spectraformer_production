...
def tune_hyperparameters(
        ...
        for batch_size in search_space['batch_size']:
            for dropout in search_space['dropout']:
                for hidden_dim in search_space['hidden_dim']:
                    ...
                    # Train for fewer epochs (just validation, not full training)
                    try:
                        trainer.train(train_loader_trial, val_loader_trial, epochs=epochs_per_trial)
                        ...
                        # Store result
                        param_key = f"lr={lr}_bs={batch_size}_do={dropout}_hid={hidden_dim}"
    ...
    if best_params:
        best_params_path = os.path.join(output_dir, f"best_hyperparameters_{dataset_name}.json")
...
def main():
    ...
    if all_best_params:
        combined_path = os.path.join(args.output_dir, "best_hyperparameters_all.json")
